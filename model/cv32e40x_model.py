...
Usage:
    ./cv32e40x_model.py <spike-trace> [--marker cycle] [--actual <rtl_cycles>] [--breakdown] [--plot]
...
try:
    ...
    from report import print_ext_breakdown, plot_top_extensions_pie
except ImportError:
    RISCVDecoder = None
    print_ext_breakdown = None
    plot_top_extensions_pie = None
...
class CV32E40XModel:
    ...
    def __init__(self, div_cycles=35, verbose=False):
        ...
        self.stalls = Counter()
        # Track branch-target misalignment penalties per target PC and source PC
        self.misaligned_targets = Counter()
        self.misaligned_sources = Counter()
        # Per-extension statistics for report.py
        self.ext_counts = Counter()
        self.ext_cycles = Counter()

    def target_penalty(self, nxt, src_ins=None):
        """+1 cycle when a control-flow target is a non-word-aligned,
        non-RVC (32-bit) instruction."""
        if nxt is None:
            return 0
        if (nxt.pc % 4) != 0 and nxt.size == 4:
            self.misaligned_targets[nxt.pc] += 1
            if src_ins is not None:
                self.misaligned_sources[src_ins.pc] += 1
            return 1
        return 0
    ...
    def insn_cycles(self, ins, nxt):
        ...
        if b in FENCES:
            self.notes["fence"] += 1
            pen = self.target_penalty(nxt, ins)
            self.stalls["fence"] += 4
    ...
    def run(self, insns):
        total = 0
        decoder = RISCVDecoder() if RISCVDecoder else None
        self.ext_counts = Counter()
        self.ext_cycles = Counter()
        ...
        for i, ins in enumerate(insns):
            ...
            # Track extension breakdown for report.py if decoder available
            if decoder:
                is_compressed = (ins.size == 2)
                tag = decoder.decode(ins.enc, is_compressed)
                parent_ext = tag.split(':')[0]
                self.ext_counts[parent_ext] += 1
                self.ext_cycles[parent_ext] += c
...
def main():
    ...
    ap.add_argument("--misalign-report", action="store_true",
                    help="report branch/jump targets causing target_misalign stall cycles")
    ap.add_argument("-v", "--verbose", action="store_true",
                    help="print per-instruction cycle costs")
    ap.add_argument("--plot", action="store_true",
                    help="display pie chart of top extensions via matplotlib")
    args = ap.parse_args()
    ...
    if args.misalign_report:
        print("\nBranch-Target Misalignment Report:")
        total_misalign = sum(model.misaligned_targets.values())
        print(f"  Total Misalignment Stalls: {total_misalign} cycles ({100.0*total_misalign/cycles:5.2f}% of execution)")
        if model.misaligned_targets:
            print(f"  {'Target PC':<12} {'Stall Cycles':<14} {'% Misalign':<12}")
            for tgt_pc, count in model.misaligned_targets.most_common(10):
                print(f"  0x{tgt_pc:08x}   {count:>10}    {100.0*count/total_misalign:6.1f}%")
        else:
            print("  No misaligned branch targets detected (all targets 32-bit word-aligned).")
    ...
    if print_ext_breakdown and model.ext_counts:
        print_ext_breakdown(model.ext_counts, model.ext_cycles, cycles)

    if args.plot and plot_top_extensions_pie:
        try:
            plot_top_extensions_pie(model.ext_cycles, cycles)
