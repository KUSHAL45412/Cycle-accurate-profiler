...
def main():
    ...
    cand_count = Counter()
    cand_cycles = Counter()
    cand_opt_saved = Counter()
    cand_expected_penalty = Counter()
    cand_worst_penalty = Counter()
    cand_example = {}
    n = len(region)
    for i in range(n):
        for L in range(2, args.max_len + 1):
            ...
            cand_count[sig] += 1
            seq_cyc = sum(cost[i:i + L])
            cand_cycles[sig] += seq_cyc

            # Microarchitectural Hazard Slack Model:
            # Check backwards within basic block for load-producer distance (d_slack).
            # d_slack == 0: already stalling in baseline.
            # d_slack == 1: 1 instruction buffer in baseline; high risk of exposing new stall after fusion.
            # d_slack >= 2: compiler has ample scheduling freedom.
            has_exposed_hazard = False
            has_any_load_producer = False
            for k in range(i - 1, max(-1, i - 5), -1):
                prv = region[k]
                if is_control(prv):
                    break
                if M.base_mnem(prv.mnem) in M.LOADS and prv.dest is not None and prv.dest in ext:
                    has_any_load_producer = True
                    d_slack = (i - 1) - k
                    if d_slack <= 1:
                        has_exposed_hazard = True
                    break

            if has_exposed_hazard:
                cand_expected_penalty[sig] += 1
            if has_any_load_producer:
                cand_worst_penalty[sig] += 1
    ...
    for sig, cnt in cand_count.items():
        ...
        cyc = cand_cycles[sig]
        # Replacing the sequence with one custom instruction costing offload_latency cycles
        opt_saved = cyc - cnt * args.offload_latency
        if opt_saved <= 0:
            continue        # offload costs more than the sequence it replaces
        exp_saved = opt_saved - cand_expected_penalty[sig]
        worst_saved = opt_saved - cand_worst_penalty[sig]
        rows.append((opt_saved, exp_saved, worst_saved, cnt, ln, nsrc, cyc, sig))
    rows.sort(key=lambda r: (r[1], r[0]), reverse=True)

    print(f"{'Opt (cyc)':>10} {'Expected':>10} {'Worst':>8} {'%Gain':>6} {'Execs':>7} {'Len':>4} {'Srcs':>5}  Pattern")
    print("-" * 88)
    for opt_saved, exp_saved, worst_saved, cnt, ln, nsrc, cyc, sig in rows[:args.top]:
        pct = 100.0 * exp_saved / total_cycles
        print(f"{opt_saved:>10} {exp_saved:>10} {worst_saved:>8} {pct:>5.1f}% {cnt:>7} "
              f"{ln:>4} {nsrc:>5}  {sig}")

    if any(cand_expected_penalty[sig] for *_, sig in rows[:args.top]):
        print("\n  [Hazard Slack Model Notes]:")
        print("  - 'Opt': Assumes compiler perfectly hides all load-use dependencies.")
        print("  - 'Expected': Compensates for load-use slack (d_slack <= 1) collapsed by fusion.")
        print("  - 'Worst': Assumes every load directly into candidate causes a 1-cycle stall.")
