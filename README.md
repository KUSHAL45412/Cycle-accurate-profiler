...
You can run the profiling pipeline in two ways: using the automated all-in-one script or step-by-step.

### Option A: All-in-One Automated Pipeline

To run the complete pipeline (Verilator simulation + VCD conversion + Hardware Profiling + Spike Trace comparison):

```bash
# Run on a specific benchmark (e.g. matmult, dotprod, or custom test)
./profile.sh matmult
```

Or run the push-button C-to-Silicon optimization and speedup flow:

```bash
python3 pariscv_flow.py --benchmark bench/matmul --idiom sdot4
```

---

### Option B: Step-by-Step Profiling Procedure

#### 1. Compile the Target Program
Compile your C or assembly test using the RISC-V GCC toolchain with the provided linker scripts:

```bash
# Build firmware for RTL simulation (generates .hex and .objdump)
riscv32-unknown-elf-gcc -march=rv32imc_zicsr_zifencei -mabi=ilp32 \
  -T model/bench/link_rtl.ld -o test_rtl.elf test.c
riscv32-unknown-elf-objcopy -O verilog test_rtl.elf test_rtl.hex
riscv32-unknown-elf-objdump -d test_rtl.elf > test_rtl.objdump

# Build firmware for Spike Golden ISS
riscv32-unknown-elf-gcc -march=rv32imc_zicsr_zifencei -mabi=ilp32 \
  -T model/bench/link_spike.ld -o test_spike.elf test.c
```

#### 2. Run Verilator RTL Simulation
Simulate the hardware core with your firmware to generate the waveform trace:

```bash
cd cores/core-v-verif/cv32e40x/sim/core
./testbench_verilator +vcd +firmware=path/to/test_rtl.hex
# Outputs verilator_tb.vcd
```

#### 3. Extract Waveform to JSON
Extract the cycle-by-cycle clock, PC, and instruction register signals from the VCD file:

```bash
python3 vcdtojson.py verilator_tb.vcd full_data.json
```

#### 4. Run the Hardware Cycle Profiler
Analyze the execution cycles, pipeline stalls, and hazard breakdowns:

```bash
python3 cv32e40x_profiler3.py full_data.json test_rtl.objdump
```

#### 5. Generate Spike ISS Golden Trace & Run Prediction Model
Generate the commit trace from the Spike simulator and evaluate the Python microarchitectural performance model:

```bash
# Generate trace
spike -l --log-commits --isa=rv32imc_zicntr_zicsr_zifencei \
  -m0xf0000:0x310000,0x80000000:0x400000 test_spike.elf > spike.txt 2>&1

# Run the cycle model and compare against actual hardware cycles
python3 model/cv32e40x_model.py spike.txt --actual <RTL_CYCLES> --breakdown
```
