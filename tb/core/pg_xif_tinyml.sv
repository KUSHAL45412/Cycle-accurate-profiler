...
//                   + (rs1[31:24] * rs2[31:24])
//
// Encoded as R4-type in RISC-V custom-0 space (0x0b):
//     rs3 = instr[31:27], funct2 = 01, funct3 = 000, opcode = 0001011 (0x0b)
...
(
  ...
  // ---------------------------------------------------------------- decode --
  localparam logic [6:0] OPCODE_CUSTOM0 = 7'b0001011; // 0x0b (custom-0)
  localparam logic [2:0] FUNCT3_COP     = 3'b000;
  localparam logic [1:0] FUNCT2_SDOT4   = 2'b01;      // 4-lane INT8 SIMD
  localparam logic [1:0] FUNCT2_SDOT2   = 2'b10;      // 2-lane INT16 SIMD
  ...
  assign is_custom_op = xif_issue_if.issue_valid && !res_pending
                        && (instr[6:0]   == OPCODE_CUSTOM0)
                        && (instr[14:12] == FUNCT3_COP);

  assign is_sdot4     = is_custom_op && (instr[26:25] == FUNCT2_SDOT4);
  assign is_sdot2     = is_custom_op && (instr[26:25] == FUNCT2_SDOT2);
  assign is_accepted  = is_sdot4 || is_sdot2;

  // ----------------------------------------------------------------- issue --
  assign xif_issue_if.issue_ready          = !res_pending;
  assign xif_issue_if.issue_resp.accept    = is_accepted;
  assign xif_issue_if.issue_resp.writeback = is_accepted;
  ...
  // --------------------------------------------------------------- compute --
  // --- 4-lane INT8 SIMD Datapath (pg.sdot4) ---
  ...
  logic signed [31:0] sum_prods_8;
  logic signed [31:0] total_sdot4;

  assign sum_prods_8 = {{16{prod0[15]}}, prod0} +
                       {{16{prod1[15]}}, prod1} +
                       {{16{prod2[15]}}, prod2} +
                       {{16{prod3[15]}}, prod3};

  assign total_sdot4 = $signed(xif_issue_if.issue_req.rs[2]) + sum_prods_8;

  // --- 2-lane INT16 SIMD Datapath (pg.sdot2) ---
  logic signed [15:0] h0_a, h1_a;
  logic signed [15:0] h0_b, h1_b;

  assign h0_a = xif_issue_if.issue_req.rs[0][15:0];
  assign h1_a = xif_issue_if.issue_req.rs[0][31:16];

  assign h0_b = xif_issue_if.issue_req.rs[1][15:0];
  assign h1_b = xif_issue_if.issue_req.rs[1][31:16];

  logic signed [31:0] prod16_0, prod16_1;
  assign prod16_0 = $signed(h0_a) * $signed(h0_b);
  assign prod16_1 = $signed(h1_a) * $signed(h1_b);

  logic signed [31:0] total_sdot2;
  assign total_sdot2 = $signed(xif_issue_if.issue_req.rs[2]) + prod16_0 + prod16_1;


  // --- Final Compute Result Multiplexer ---
  logic signed [31:0] compute_result;
  assign compute_result = is_sdot2 ? total_sdot2 : total_sdot4;
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      res_rd      <= '0;
      res_id      <= '0;
    end else if (is_accepted) begin
      res_pending <= 1'b1;
      res_data    <= compute_result;
      res_rd      <= instr[11:7];
